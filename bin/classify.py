#!/usr/bin/env python3
"""
Binary classifier for ALS vs Control using insert size features
"""
import sys
import pandas as pd
import numpy as np
...
def main(summary_file, output_file):
    ...
    # Features: mean, median, stddev, q25, q75
    feature_cols = ['mean', 'median', 'stddev', 'q25', 'q75']
    ...
    for name, clf in classifiers.items():
        ...
        # Use stratified K-fold cross-validation (k=5 or leave-one-out for small datasets)
        # With 22 samples, let's use k=5
        ...
        recall = recall_score(y, y_pred, zero_division=0)  # Also called sensitivity
        f1 = f1_score(y, y_pred, zero_division=0)
        
        print(f"Accuracy:    {accuracy:.3f}")
        print(f"Precision:   {precision:.3f}")
        print(f"Sensitivity: {recall:.3f}")  # Recall = Sensitivity
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: classify.py <summary_file> <output_file>")
        sys.exit(1)
